...
function LoadVidyoClientLibrary(webrtc, joincall, room, user, isCaller) {
    ...
    script.src = 'https://static.vidyo.io/4.1.23.21/javascript/VidyoClient/VidyoClient.js?useNativeWebRTC=true&onload=onVideoClientLoaded&webrtc=' + webrtc + '&plugin=false' + webrtcLogLevel;
    //alert(script);
...
function JoinCall()
{
    ...
    _vidyoConnector.Connect({
        ...
        onSuccess: function () {
            ...
            PlayRingBack(false);
            $('#chkSecondLanguage').prop('checked', true).trigger("change");
