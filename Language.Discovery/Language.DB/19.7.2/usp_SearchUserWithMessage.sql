...
begin
	...
	select distinct  x.UserID, UserName,FirstName, LastName, C.CityName + ',' + x.CountryName as [Address], isnull(Avatar,'') Avatar, StatusText, 
		IsOnline, x.CreateDate, isnull(xx.UnReadMessageCount,0) UnReadMessageCount, SchoolID, r.x IsReplied,dr.RepliedDate, isnull(x.IsCantalk,0) IsCanTalk
	from
	(
	SELECT u.UserID,UserName,FirstName, LastName, U.[Address], isnull(Avatar,'') Avatar, StatusText, 
		...
		,City.CityHeaderID, Country.CountryName, si.SchoolID, u.IsCanTalk--,  1 IsReplied  --dbo.ufn_IsLastMessageMine(u.UserID, @SearcheeID) as IsReplied 
		FROM [User] U
		...
		group by u.UserID,UserName,FirstName, LastName, U.[Address], Avatar, StatusText, IsOnline, City.CityHeaderID, Country.CountryName,si.SchoolID, u.IsCanTalk
	) x
