...
BEGIN
	...
	BEGIN
		...
		(
			...
			WHERE ((@Word is null OR P.Word like + N'%' + @Word + '%')  OR (@Keyword is null OR S.Keyword like + N'%' +  @Keyword + '%') )
	...
	Set @MainSQL = ';WITH PS AS 
		(
			SELECT Distinct x.PaletteID, ROW_NUMBER() OVER (ORDER BY  (select 1)) AS RowNum 
			FROM (
