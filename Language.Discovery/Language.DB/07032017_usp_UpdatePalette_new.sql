...
begin
    ...
    SELECT PhraseID, SentenceID,WordMapID,Word,Keyword, WordSoundFile, WordImageFile,Ordinal
    FROM @Phrase
	where PhraseID <= 0
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @SentenceID, WordMapID, Word,Keyword, WordSoundFile, WordImageFile, Ordinal
			FROM @NewPhrase
	...
	--update old phrase
		UPDATE Phrase SET Word = temp.Word, WordSoundFile = temp.WordSoundFile, 
			WordImageFile = temp.WordImageFile, Ordinal = temp.Ordinal
		from Phrase P
		inner join @Phrase temp on P.PhraseID = temp.PhraseID	
