...
BEGIN
	...
	BEGIN
		...
		WHERE Grade between @Grade - 2 AND @Grade + 2  AND U.CountryID <> @CountryId AND U.NativeLanguage <> @NativeLanguage AND U.UserTypeID = 3
		ORDER BY NEWID()  
		...
		BEGIN
			UPDATE Schedule SET PartnerId = @UserId, PartnerConfirmationToken = NEWID() WHERE ScheduleId = @ScheduleId
			
			SET @ID = @ScheduleId
		END
		ELSE
		BEGIN
			INSERT INTO Schedule(Schedule, UserId, CreateDate, UserConfirmationToken)
				VALUES( @Date, @UserId, GETDATE(), NEWID() )
