...
begin
	...
	IF @LevelID = 4 
	BEGIN
		select temp.*, ISNULL(U.StatusText ,'') StatusText
		from
		(
			select top (@Top) x.UserID,FirstName, LastName, IsNull(C.CityName,'') + '<br/>' + isnull(x.CountryName,'') as [Address], Avatar, LastLogin, x.MessageCount, CityID , Gender , UserName
			from 
			(
