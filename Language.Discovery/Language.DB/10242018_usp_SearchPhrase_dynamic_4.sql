...
GO
/****** Object:  StoredProcedure [dbo].[usp_SearchPhrase_dynamic_4]    Script Date: 10/24/2018 9:46:02 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	set @ParamDefinition = '@RowsPerPage INT, @PageNumber INT, @Keyword NVARCHAR(1000), 
		...
		@SchoolPalette bit, @TopCategoryHeaderID int, @IsTalk bit, @SentenceLink nvarchar(50), @VirtualCount int output'
	...
	begin
		...
		IF ISNULL(@SentenceLink,'') <> ''
		BEGIN
			Set @MainSQL = 'INSERT INTO #PaletteTemp
				...
				INNER join PaletteLink PLL on PL.PaletteID = PLL.PaletteID
				...
				AND PLL.SentenceLink like ''%'' + @SentenceLink + ''%''
				group by PL.PaletteID
				ORDER BY MAX(PLL.SentenceLink )
				
				SELECT @GroupID
				SELECT * FROM #PaletteTemp '
	  ...
	  execute sp_executesql @MainSQL,@ParamDefinition,@RowsPerPage, @PageNumber, @Keyword, 
		@CategoryID,@SchoolID, @LevelID, @GroupID, @SchoolPalette, @TopCategoryHeaderID, @IsTalk,  @SentenceLink, @VirtualCount = @VirtualCount output
