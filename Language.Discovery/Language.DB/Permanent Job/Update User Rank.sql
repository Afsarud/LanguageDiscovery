...
/****** Object:  Job [Update User Rank]    Script Date: 1/31/2020 1:15:18 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 1/31/2020 1:15:18 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [step1]    Script Date: 1/31/2020 1:15:18 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'step1', 
	...
	WHERE  U.LevelID not in (3,4)
	and u.CountryID = 1 AND UM.CreateDate>=DATEADD(DAY,-7,GETDATE())
	--GROUP BY U.USerID
	...
	INNER JOIN [user] u on UM.SenderID = u.UserID
	WHERE  (U.LevelID not in (3,4))
	and u.CountryID = 2 AND UM.CreateDate>=DATEADD(DAY,-7,GETDATE())
	
	IF  @JPMessageCount <> 0 AND @AUMessageCount * 100.0/@JPMessageCount < 50
		SET @Multiplier = 10
	...
	(
		...
		WHERE UM.SenderID > 0 and IsLike = 1 AND U.LevelID not in (3,4)
		AND U.IsActive = 1
	...
	(
		...
		INNER JOIN [Country] C on u.CountryID = C.CountryID
		INNER JOIN [SchoolInfo] S on U.SchoolID = S.SchoolID
		WHERE UM.SenderID > 0 and IsLike = 0 AND U.LevelID not in (3,4)
		AND U.IsActive = 1
		GROUP BY U.USerID,U.UserName,C.Flag, S.Name1 , U.Avatar, S.SchoolCode, U.FirstName, U.CountryID
		) xx
		inner JOIN 
	...
	where x.UserID not in (select UserID1 from @UserRank)
	ORDER BY StarCount DESC
