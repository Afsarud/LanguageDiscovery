...
BEGIN
	...
	BEGIN
		...
		BEGIN
			UPDATE Schedule SET PartnerId = @UserId, PartnerConfirmationToken = NEWID(), PartnerPhraseCategoryID = @CategoryId
		...
		if exists(select UserID from UserTalkSubscription where userId = @UserId and IsActive = 1)
		begin	
			UPDATE UserTalkSubscription SET SessionTime = 31, BalanceTime = 31, TotalTime = 31 where UserID = @UserID and IsActive = 1
		end
		else
		begin
			INSERT INTO UserTalkSubscription 
			VALUES (@UserID, 31, 31, 31, getdate(), 1)
		end
	END
