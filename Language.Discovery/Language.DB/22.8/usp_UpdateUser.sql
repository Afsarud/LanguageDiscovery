ALTER procedure [dbo].[usp_UpdateUser]
 @ID int
   ...
   ,@BalanceTime int = 0
   ,@SessionTime int = 0
   ,@NumberOfMatching int = 0
   ,@OrderByLearningLanguageFlag bit = 0
   ,@LinkKey nvarchar(20) = null
   ,@IsParentsInfoStored bit = 0
...
begin
	...
	WHERE SI.SchoolID = @SchoolID
	 ...
	 if @CountryID IN ( 1, 4) -- australia quickfix
	 begin
 ...
 SET [UserName] = @UserName
  ...
  ,TrialExpirationDate = @TrialExpirationDate
   ...
   ,PasswordExpiration = case when @Password = Password THEN PasswordExpiration ELSE DATEADD(d, -1, getdate()) END
   ,LinkKey = @LinkKey
   ,IsParentsInfoStored = @IsParentsInfoStored
 ...
 IF EXISTS(Select * FROM UserTalkSubscription where UserID = @ID and IsActive = 1)
	 BEGIN
