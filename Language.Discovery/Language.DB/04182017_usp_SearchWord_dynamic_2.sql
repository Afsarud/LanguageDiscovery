ALTER PROCEDURE [dbo].[usp_SearchWord_dynamic_2]
	@SchoolID bigint = 0,
	@RowsPerPage INT = 10,
	@PageNumber as INT = 1,
...
begin	
	...
	INSERT INTO @Result 
	SELECT * from ufn_Split(@Keyword, default)
	...
	begin
		...
		select @exactcriteria = @exactcriteria + 'W.Word =N''' + Keyword + ''' OR ' from  @Result where ID = @id
		set @id = @id + 1
	end
	print @criteria
	...
	(
		...
		WHERE (ISNULL(@Keyword,'''') = '''' ' + @exactcriteria + ')
