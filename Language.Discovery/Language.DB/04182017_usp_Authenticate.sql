...
BEGIN
	...
	SELECT @UserID = U.UserID, @SchoolID = U.SchoolID, @UserTypeID = U.UserTypeID, 
		...
		WHERE UserName = @UserName AND U.[Password] = @Password	
	...
	WHERE UserName = @UserName AND [Password] = @Password
		...
		IF ( EXISTS (SELECT 1 
					...
					WHERE UserName = @UserName AND [Password] = @Password) )
					BEGIN
						UPDATE [User] set LastLogin = GETDATE(), IsOnline = Case when @IsAdminLogin = 1 then 0 else 1 end , IsActive=1, HasAgreedTC = 1 WHERE UserName = @UserName AND [Password] = @Password
