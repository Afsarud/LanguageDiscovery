ALTER PROCEDURE [dbo].[usp_SearchWord_dynamic_UserCreated]
	@SchoolID bigint = 0,
	@RowsPerPage INT = 10,
	@PageNumber as INT = 1,
...
begin	
	...
	declare @WordTypes as TABLE
	(
		ID int identity(1,1),
		WordType nvarchar(50)
	)
	INSERT INTO @WordTypes
	SELECT * from ufn_Split(@WordType, ';')

	declare @criteria nvarchar(max)
	declare @exactcriteria nvarchar(max)
	declare @likecriteria nvarchar(max)
	declare @wordtypecriteria nvarchar(max) = ''
	declare @id int
	declare @count int
	set @id = 1
	...
	select top 1 @id =  id from @WordTypes order by id
	select @count = max(ID) from @WordTypes
	
	while @id <= @count
	begin
		
		select @wordtypecriteria = @wordtypecriteria + 'WH.WordType like N''%' + replace(WordType ,'''''''''','''''') + '%'' OR ' from  @WordTypes where ID = @id
		set @id = @id + 1
	end


	set  @criteria = ''--case when len(@criteria) > 0 then ' OR (' + reverse(stuff(reverse(@criteria), 1, 3, '')) + ')' else '' end
	set  @wordtypecriteria = case when len(@wordtypecriteria) > 0 then ' OR (' + reverse(stuff(reverse(@wordtypecriteria), 1, 3, '')) + ')' else '' end
	...
	(
		...
		AND (ISNULL(@WordType,'''') = '''' ' + @wordtypecriteria + ')
		AND WH.CreatedByID = @SearcheeID
		AND WH.UserCreatedWord = 1
	...
	--select * from Y
	select @VirtualCount = COUNT(*) from Y
	...
	@SchoolPalette bit, @FirstQueryCount int, @TopCategoryHeaderID int, @IsTalk bit, @WordType nvarchar(50), @SearcheeID bigint , @VirtualCount int output'

		
	  execute sp_executesql @MainSQL,	@ParamDefinition,@RowsPerPage, @PageNumber, @Word,@Keyword, 
	@CategoryID,@SchoolID, @GroupID, @SchoolPalette, @FirstQueryCount, @TopCategoryHeaderID,@IsTalk, @WordType, @SearcheeID, @VirtualCount = @VirtualCount output
