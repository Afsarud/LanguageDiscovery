...
begin
	...
	BEGIN
		...
		DELETE Word where WordMapID in (select WordHeaderid from WordHeader where isnull(PhraseCategoryID,0) = 0 AND UserCreatedWord = 0 )
		DELETE WordHeader where isnull(PhraseCategoryID,0) = @PhraseCategoryID AND UserCreatedWord = 0
	END
	...
	BEGIN
		...
		INSERT INTO WordHeader ( CreateDate, CreatedByID, ImageFile, PhraseCategoryID, KeyWord, WordType, Sequence )
		SELECT CreateDate, CreatedByID, ImageFile, PhraseCategoryID, KeyWord, WordType, Sequence
		FROM @WordHeader 
		where ID = @ID
