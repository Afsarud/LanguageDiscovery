alter PROCEDURE [dbo].[usp_SearchPhrase_dynamic_4]
	@RowsPerPage INT = 10, 
	@PageNumber INT = 1, 
...
BEGIN
	...
	while @id <= @count
	begin
		--select @criteriaWord = @criteriaWord + 'Word = N''' + Keyword + ''' OR ' from  @Result where ID = @id
		select @criteriaKeyword = @criteriaKeyword + 'S.Keyword Like N''%' + replace(Keyword ,'''''''''','''''') + '%'' OR ' from  @Result where ID = @id
		--select @criteriaKeyword = @criteriaKeyword + 'S.Keyword Like N''%' + Keyword + '%'' OR Word Like N''%' + Keyword + '%'' OR ' from  @Result where ID = @id
		set @id = @id + 1
	end
	...
	--;WITH PS AS 
	--(
	--	SELECT Distinct x.SentenceID, ROW_NUMBER() OVER (ORDER BY SentenceID) AS RowNum 
	--	FROM (SELECT Distinct P.SentenceID
	--	FROM Phrase P
	--	INNER JOIN Sentence S on P.SentenceID = S.SentenceID
	--	WHERE (@Word is null OR P.Word like @Word + '%')  AND (@Keyword is null OR P.Keyword like @Keyword + '%') 
	--	AND @CategoryID = 0 OR P.PhraseCategoryID = @CategoryID) X
	--)
	--INSERT INTO @Sentences
	--SELECT PS.SentenceID, S.ImageFile, S.SoundFile, S.DefaultLanguageCode
	--FROM PS 
	--INNER JOIN Sentence S ON PS.SentenceID = S.SentenceID
	--WHERE PS.RowNum BETWEEN ((@PageNumber-1)*@RowsPerPage)+1
	--AND @RowsPerPage*(@PageNumber) 
	--ORDER BY Ps.SentenceID
	...
	--	FROM Phrase P
	--	INNER JOIN Sentence S on P.SentenceID = S.SentenceID
	--	WHERE (@Word is null OR P.Word like @Word + '%')  
	--		AND (@Keyword is null OR P.Keyword like @Keyword + '%') 
	--		AND @CategoryID = 0 OR P.PhraseCategoryID = @CategoryID) X
	--)
	--SELECT P.SentenceID, P.WordGroupID, P.LanguageCode, P.Word, P.Keyword, P.PluralForm, P.SchoolID,
	--		P.PhraseCategoryID, P.SoundFile, P.ImageFile, P.Ordinal
	--FROM PS 
	--INNER JOIN Phrase P
	--on PS.SentenceID = P.SentenceID
	--WHERE PS.RowNum BETWEEN ((@PageNumber-1)*@RowsPerPage)+1
	--AND @RowsPerPage*(@PageNumber) 
	--ORDER BY P.SentenceID
